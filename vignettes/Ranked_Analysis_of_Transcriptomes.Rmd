---
title: "RAT: Ranked Analysis of Transcriptomes"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{RAT: Ranked Analysis of Transcriptomes}
...
Need a high level summary here.

Another nice feature about this package is that it can handle messy data and automatically performs gene name conversion, making it very suitable for exploratory data analysis.

To be more specific, it takes two steps to perform ranked analysis of transcriptome: build a scaffold PCA space, then project your new samples onto the PCA space. 
...
You can also turn on the "tiny_label" mode by specifying `plot_mode`. You can also visualize other principle components by specifying `pcs`.  
...
Then call:
```{r fig3, fig.height = 4, fig.width = 6}
...
Then run:
```{r fig5, fig.height = 4, fig.width = 6}
