...
test_that("loadingPlot works", {
    ...
    expect_s3_class(loadingPlot(scaffold, dims = c(2,3)), "ggplot")
    expect_s3_class(loadingPlot(scaffold, dims = c(2,3), num_genes = 5), "ggplot")
    expect_warning(loadingPlot(scaffold, dims = c(2,3), gene_name = "entrez"))
    expect_s3_class(loadingPlot(scaffold, dims = c(2,3), angle = 45), "ggplot")
})


test_that("loadingPlot returns righ colnames", {
    df <- loadingPlot(scaffold, dims = c(2,3), df_only = TRUE, num_genes = 4)
