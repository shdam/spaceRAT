test_that("buildScaffold() returns an scaffoldSpace object",{
  ...
  space5 <- buildScaffold(exprs_dmap,pData_dmap,"cell_types",plot_mode="tiny_label",pcs=c(1,2))
  expect_s4_class(space5,"scaffoldSpace")

  space6 <- buildScaffold(exprs_dmap,pData_dmap,"cell_types",plot_mode="tiny_label",classes=c("HSC","MEP","ERY"),pcs=c(1,2))
